...
#ifndef STATE_CHART_INTERNAL_MODEL_RAISE_H_
#define STATE_CHART_INTERNAL_MODEL_RAISE_H_
...
class Raise : public ExecutableContent {
 public:
  explicit Raise(const string& event);
  Raise(const Raise&) = delete;
  Raise& operator=(const Raise&) = delete;

  // Enqueues an internal event by calling runtime->EnqueueInternalEvent().
...
#endif  // STATE_CHART_INTERNAL_MODEL_RAISE_H_
